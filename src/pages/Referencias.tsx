import { PageContainer } from "@/components/layout/PageContainer";
  ...
  export default function Referencias() {
    return (
      ...
      <p dangerouslySetInnerHTML={{ __html: "Quando você arrasta uma fórmula, o Excel <strong>ajusta as referências automaticamente</strong>. Às vezes isso é o que queremos, às vezes não. O cifrão (<code>$</code>) trava o que vier na frente dele." }} />
...
Mista      |  A\$1     | muda    | trava`} />
      <AlertBox type="tip" title="Atalho indispensável: F4">
        <span dangerouslySetInnerHTML={{ __html: "Com o cursor sobre uma referência na barra de fórmulas, pressione <code>F4</code> repetidamente para alternar: A1 → $A$1 → A$1 → $A1 → A1." }} />
      </AlertBox>
      <h2>Exemplo: tabuada</h2>
      <p dangerouslySetInnerHTML={{ __html: "Construir uma tabuada 1×1 a 10×10 com uma única fórmula. Linha 1 tem 1..10, coluna A tem 1..10. Em B2:" }} />
...
=[Outra.xlsx]Plan1!C3         → outra pasta de trabalho aberta`} />
      <h2>Referência 3D</h2>
      <p dangerouslySetInnerHTML={{ __html: "Soma a mesma célula em várias planilhas:" }} />
