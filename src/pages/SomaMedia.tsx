import { PageContainer } from "@/components/layout/PageContainer";
  ...
  =MODO.ÚNICO(B2:B100)        → moda (mais frequente)`} />
      ...
      <ul>
        <li dangerouslySetInnerHTML={{ __html: "<code>CONT.NÚM(A1:A100)</code> — conta apenas números." }} />
        <li dangerouslySetInnerHTML={{ __html: "<code>CONT.VALORES(A1:A100)</code> — conta tudo que não é vazio (texto + número)." }} />
        <li dangerouslySetInnerHTML={{ __html: "<code>CONTAR.VAZIO(A1:A100)</code> — conta células vazias." }} />
      </ul>
  ...
  =MENOR(B2:B100;2)     → segundo menor valor`} />
      <h2>AutoSoma — Alt + =</h2>
      <p dangerouslySetInnerHTML={{ __html: "Selecione a célula vazia abaixo (ou à direita) de uma sequência de números e pressione <code>Alt + =</code>. O Excel insere automaticamente <code>=SOMA(...)</code> com o intervalo certo." }} />
      <AlertBox type="tip" title="SOMA ignora texto">
        <span dangerouslySetInnerHTML={{ __html: "Se houver texto no intervalo, <code>SOMA</code> ignora silenciosamente. Já <code>=A1+A2</code> dará <code>#VALOR!</code> se A1 ou A2 forem texto." }} />
      </AlertBox>
